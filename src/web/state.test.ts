...
test("loadState", () => {
  const cases: Array<{ name: string; stored: Record<string, string>; want: PersistedState | undefined }> = [
    ...
    { name: "unrecognized screen: no saved state", stored: { "rumble31.state": '{"version":1,"state":{"screen":"bogus"}}' }, want: undefined },
    { name: "screen missing entirely: no saved state", stored: { "rumble31.state": '{"version":1,"state":{}}' }, want: undefined },
    { name: "valid main screen", stored: { "rumble31.state": '{"version":1,"state":{"screen":"main"}}' }, want: { screen: "main" } },
    { name: "valid settings screen", stored: { "rumble31.state": '{"version":1,"state":{"screen":"settings"}}' }, want: { screen: "settings" } },
    { name: "valid about screen", stored: { "rumble31.state": '{"version":1,"state":{"screen":"about"}}' }, want: { screen: "about" } },
    {
      name: "valid game screen",
      stored: { "rumble31.state": JSON.stringify({ version: 1, state: { screen: "game", game: sampleGame } }) },
      want: { screen: "game", game: sampleGame },
    },
    {
      name: "valid over screen",
      stored: { "rumble31.state": JSON.stringify({ version: 1, state: { screen: "over", game: sampleOver } }) },
