...
// Match plays a series of games among four fixed seats until only one
// remains active, per specs/rules.md. Once a seat is eliminated it is
// left out of every later game entirely — not dealt in, never takes a
...
export class Match {
  ...
  // active seat at once, ending the match in a tie instead of
  ...
  // are actually eliminated — the match ends instead with them tied as
...
// newMatch returns a Match ready to play, deriving every game's seed
// from seed so a match is fully reproducible. initialStrikes seeds each
// seat's strike count (for -strikes debugging); a seat starting at 3 or
// more strikes begins the match already eliminated, per applyResult's
// own threshold.
export function newMatch(
  ...
  return new Match({ strategies, rng: new Rng(seed), strikes: [...initialStrikes], eliminated });
}
