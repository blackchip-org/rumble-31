---
name: release-notes
description: Generate user-facing release notes from the diff between the latest git tag and the one before it, then (after the user approves the wording) prepend a new dated section to CHANGELOG.md. Use when the user asks to write, generate, or draft release notes, or to update the changelog for the latest release. Notes describe what a player of the app would notice — new features, rule/UI changes, bug fixes — not internal refactors, tooling, tests, docs, or dev-only scripts.
---

# Release Notes

Draft a concise, player-facing summary of what changed between the two
most recent git tags, get the user's sign-off on the wording, then
prepend it to `CHANGELOG.md` at the repo root.

## Step 1: Find the latest and previous tag

```bash
git for-each-ref refs/tags --sort=-creatordate --format='%(refname:short) %(creatordate:short)'
```

Take the top two lines as the latest tag and the previous tag. Sort by
creation date, not tag-name string order, since that's robust even if
tags aren't named in strict sequence.

If there are fewer than two tags, stop and tell the user there's nothing
to diff against yet.

## Step 2: Check for an already-existing entry

Read `CHANGELOG.md` if it exists. If it already has a section for the
latest tag's version, tell the user and ask whether to regenerate
(replace) that section or stop — don't silently duplicate or overwrite.

## Step 3: Gather the changes

```bash
git log <previous>..<latest> --oneline
```
...
Use the version number from `src/version.ts` as checked out at the
latest tag (`git show <latest>:src/version.ts`), and the latest tag's
creation date from Step 1, to build the section header:
